...
class BaseController:
    ...
    def _check_enabled(self):
        """Verifica se o controller está habilitado via feature flags."""
    ...
    async def _test_slack_connection(self) -> bool:
        ...
        try:
            # Envia mensagem de health check
    ...
    async def _send_compliance_issues_notification(self, body: Dict[str, Any], report: ComplianceReport) -> None:
        ...
        if critical_issues:
            ...
            for issue in critical_issues[:5]:  # Limita a 5 para não exceder limite do Slack
        ...
        # Adiciona recomendações principais
        if report.recommendations:
            message += "*💡 Principais Recomendações:*\n"
            for rec in report.recommendations[:3]:  # Limita a 3 recomendações principais
                if rec.startswith("**"):  # Título de categoria
                    message += f"\n{rec}\n"
                else:
                    message += f"• {rec}\n"
    ...
    async def _send_compliance_warnings_notification(self, body: Dict[str, Any], report: ComplianceReport) -> None:
        ...
        if report.warnings:
            ...
            for warning in report.warnings[:5]:  # Limita a 5 warnings
        ...
        # Adiciona recomendações se houver
        if report.recommendations:
            message += "\n*💡 Recomendações:*\n"
            for rec in report.recommendations[:2]:  # Limita a 2 recomendações
                if not rec.startswith("**"):  # Ignora títulos de categoria
                    message += f"• {rec}\n"
    ...
    def _build_compliance_summary_fields(self, report: ComplianceReport) -> List[Dict[str, str]]:
        ...
        # Adiciona contagem de issues/warnings se houver
        if report.issues:
