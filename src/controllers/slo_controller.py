...
class SLOController(BaseController):
    ...
    def _determine_slo_severity(self, result: Dict[str, Any]) -> NotificationSeverity:
        ...
        if has_validation_errors and any("será ignorado" not in e for e in result["validation_errors"]):
            return NotificationSeverity.ERROR
    ...
    def _determine_slo_channel(self, severity: NotificationSeverity, result: Dict[str, Any]) -> NotificationChannel:
    ...
    def _build_slo_additional_fields(
        ...
        fields = [
            ...
            {"title": "Status", "value": "✅ Sucesso" if result["success"] else "❌ Falha", "short": True},
        ...
        if spec:
            ...
            fields.append({"title": "Target", "value": f"{spec.target}%", "short": True})
        ...
        if validation_errors:
            ...
            if errors:
                fields.append({"title": "Erros Validação", "value": str(len(errors)), "short": True})
            if warnings:
                fields.append({"title": "Warnings", "value": str(len(warnings)), "short": True})
    ...
    def _update_status_with_error(self, body: Dict[str, Any], error_msg: str, context: Dict[str, Any]):
        """
...
async def on_slo_create(body, **kwargs):
    ...
    """
    return await slo_controller.on_slo_config_change(body, event_type="create", **kwargs)
...
async def on_slo_update(body, **kwargs):
    ...
    Returns:
        Resultado da operação
    """
    return await slo_controller.on_slo_config_change(body, event_type="update", **kwargs)
