...
class DatadogRepository(DatadogPort):
    def __init__(
        self, api_key: str, app_key: Optional[str] = None, site: str = "datadoghq.com"
    ):
    ...
    def get_service_definition(self, service_name: str) -> Optional[ServiceDefinition]:
        self.logger.info(
            "Buscando definição de serviço", extra={"service_name": service_name}
        )
    ...
    def get_service_slos(self, service_name: str) -> List[SLO]:
        self.logger.info(
            "Buscando SLOs do serviço", extra={"service_name": service_name}
        )
        ...
        try:
            ...
            self.logger.info(
                "SLOs encontrados",
                extra={"service_name": service_name, "count": len(slos)},
            )
        ...
        try:
            ...
            if slo_id:
                self.logger.info(
                    ...
                    extra={"slo_id": slo_id, "slo_name": slo.name},
                )
    ...
    def update_slo_apps(self, slo_id: str, slo: SLO) -> bool:
        self.logger.info(
            "Atualizando SLO",
            extra={"slo_id": slo_id, "slo_name": slo.name, "service": slo.service_name},
        )
        ...
        try:
            ...
            self.logger.exception("Erro ao atualizar SLO", extra={"slo_id": slo_id})
            return False
    ...
    def _extract_slo_id_from_response(self, response: Dict[str, Any]) -> Optional[str]:
        try:
            ...
            self.logger.exception(
                "Falha ao extrair SLO ID", extra={"response": str(response)[:500]}
            )
            return None
