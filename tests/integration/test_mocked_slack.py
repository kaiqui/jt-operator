...
class TestMockedSlackIntegration:
    ...
    def mock_slack_clients(self):
        ...
        with patch('slack_sdk.web.async_client.AsyncWebClient') as mock_web_client:
            with patch('slack_sdk.webhook.async_client.AsyncWebhookClient') as mock_webhook_client:
                # Mock bot client
                mock_bot = AsyncMock()
    ...
    async def test_slack_repository_integration(self, mock_slack_clients):
        ...
        # Send notification
        from src.domain.slack_models import SlackNotification, NotificationSeverity, NotificationChannel
